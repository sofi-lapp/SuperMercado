...
public class SMSObservador implements IObservadorPedido {

    private static final Logger log = LoggerFactory.getLogger(SMSObservador.class);
    ...
    public void actualizar(Pedido pedido, EstadoPedidoEnum nuevoEstado) {
        ...
        log.info("[SMS] Para: {} | {}", pedido.getCliente().getNombre(), mensaje);
        persistirNotificacion(pedido, mensaje);
    }

    private String construirMensaje(Pedido pedido, EstadoPedidoEnum nuevoEstado) {
        return String.format("E-Market: Tu pedido #%d está %s", pedido.getId(), nuevoEstado);
    }
    ...
    private void persistirNotificacion(Pedido pedido, String mensaje) {
        ...
        notificacion.setTipo(TipoNotificacion.SMS);
